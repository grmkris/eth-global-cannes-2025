...
import { getContractAddress, networkConfigs } from '../../network-config'
...
export function LedgerFullDemo() {
  ...
  const handleCreateAuthorization = async () => {
    ...
    try {
      ...
      const contractAddress = networkConfigs[sepolia.id]?.webAuthnDelegationAddress || '0x0000000000000000000000000000000000000000'
      ...
      setProcessError(err instanceof Error ? err.message : 'Failed to create authorization')
    } finally {
  ...
  const handleSendTransaction = async () => {
    ...
    try {
      ...
      setProcessError(err instanceof Error ? err.message : 'Failed to send transaction')
    } finally {
      setIsProcessing(false)
    }
  }
  ...
  const handleInitializeDelegation = async () => {
    ...
    try {
      ...
      setProcessError(err instanceof Error ? err.message : 'Failed to initialize delegation')
    } finally {
      setIsProcessing(false)
    }
  }
      ...
      <CardContent className="space-y-6">
        ...
        {step === 'connect' && (
          <div className="space-y-4">
            <div className="flex items-center gap-2 text-muted-foreground">
              <div className={`w-8 h-8 rounded-full flex items-center justify-center ${isConnected ? 'bg-green-500 text-white' : 'bg-muted'}`}>
                1
              </div>
        ...
        {step === 'authorize' && (
          <div className="space-y-4">
            ...
            <div className="p-3 bg-muted rounded-md text-xs">
              <div><strong>Contract:</strong> {networkConfigs[sepolia.id]?.webAuthnDelegationAddress || 'Not configured'}</div>
              <div><strong>Chain:</strong> {sepolia.name}</div>
              <div><strong>EOA:</strong> {accountAddress}</div>
            </div>
        ...
        {step === 'execute' && authorization && (
          <div className="space-y-4">
            <div className="flex items-center gap-2 text-green-600">
              <CheckCircle className="h-4 w-4" />
              <span className="text-sm font-medium">Authorization Created!</span>
            </div>
            ...
            <div className="space-y-3 p-4 border rounded-lg">
              <h4 className="font-medium">Option 1: Initialize Delegation Contract</h4>
              <p className="text-sm text-muted-foreground">
                Initialize the delegation contract with a passkey public key (one-time setup)
              </p>
              <Button 
